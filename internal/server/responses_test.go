...
func TestProxyResponsesTextStream(t *testing.T) {
	...
	for _, want := range []string{"response.created", "response.output_item.added", `"content":[]`, `"delta":"O"`, `"delta":"K"`, "response.output_item.done", "response.completed"} {
		assert.Contains(t, body, want)
	}
	assert.Less(t, strings.Index(body, "response.created"), strings.Index(body, "response.completed"))
}
